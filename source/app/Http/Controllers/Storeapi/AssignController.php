...
class AssignController extends Controller
{
    public function delivery_boy_list(Request $request)
    {
        ...
        $nearbydboy = DB::table('delivery_boy')
            ...
            ->select("delivery_boy.boy_name", "delivery_boy.dboy_id", "delivery_boy.lat", "delivery_boy.lng", "delivery_boy.boy_city", "wallet", DB::raw("Count(orders.order_id)as count"), DB::raw("6371 * acos(cos(radians(" . $store->lat . ")) 
       ...
       $credit_limit
        = DB::table('freedeliverycart')
            ->first();
        
        if (count($nearbydboy) > 0) {
        foreach ($nearbydboy as $dboy) {
            if ($dboy->wallet>$credit_limit->credit_limit) {
                  $dboydata[] = $dboy;
                    $message = array('status' => '1', 'message' => 'Delivery Boy data', 'data' => $dboydata);
                    return $message;
            } else {
                 $message = array('status' => '0', 'message' => 'Delivery boy has exceeded minimum credit');
            return $message;
            }
    ...
    public function storeconfirm(Request $request)
    {
        ...
        if ($orderconfirm) {
            ...
            $message = array('status' => '0', 'message' => 'Already Assigned to ' . $getDevice->boy_name . 'or pending from deliveryboy');
            return $message;
        }
    }
}
