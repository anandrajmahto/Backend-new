...
class DriverorderController extends Controller
{
    ...
    public function delivery_completed(Request $request)
    {
        ...
        $ord = DB::table('orders')
            ->where('cart_id', $cart_id)
            ->first();
        $d_charge = DB::table('freedeliverycart')
            ->first();
        $store_id = $ord->store_id;
        $user_id = $ord->user_id;
        // cod due amount addint to ewallet wallet
        ...
        if ($ord->payment_method == "COD" || $ord->payment_method == "cod") {
            $dboywallet
                ...
                ->where('dboy_id', $ord->dboy_id)
                ->update([
                    'wallet' => (($dboy->wallet) - $ord->total_price) + $d_charge->del_charge
                ]);
        } else {
            $dboywallet
                = DB::table('delivery_boy')
                ->where('dboy_id', $ord->dboy_id)
                ->update([
                    'wallet' => ($dboy->wallet) + $d_charge->del_charge
                ]);
        }
