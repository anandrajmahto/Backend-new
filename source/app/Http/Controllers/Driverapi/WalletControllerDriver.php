...
class WalletControllerDriver extends Controller
{
   use SendSms;
   use SendMail;
    ...
    {  
        ...
        $wallet = DB::table('delivery_boy')
                ->select('wallet')
                ->where('dboy_id', $user_id)
                ->first();
    ...
    { 
        ...
        $user_id = $request->user_id;
        $wallet_amt = DB::table('delivery_boy_new')
                    ->select('wallet')
                    ->where('dboy_id', $user_id)
                    ->first();
...
if($recharge_status == 'success'){
             $wallet_amt = DB::table('delivery_boy_new')
              ...
              $insert=  DB::table('wallet_recharge_history_dboy')
                    ->insert(['dboy_id'=>$user_id,
        ...
        if($insert){
            ...
            //abhishek fix this and delete this comment after it is fixed 
            //sms feature creditals and functionality 
    ...
    { 
       ...
       $last = DB::table('wallet_recharge_history_dboy')
    ...
    { 
        $user_id = $request->user_id;
        $show =  DB::table('wallet_recharge_history_dboy')
