...
class DriverloginController extends Controller
{
 public function driver_login(Request $request)
    ...
    public function driver_login_new(Request $request)
    
     {
    	$phone = $request->phone;
    	$password = $request->password;
    	$device_id = $request->device_id;
    	$checkdriver1 = DB::table('delivery_boy_new')
    					->where('boy_phone', $phone)
    					->first();
    if($checkdriver1){					
    	$checkdriver = DB::table('delivery_boy_new')
    					->where('boy_phone', $phone)
    					->where('password', $password)
    					->first();

    	if($checkdriver){
    		   $updateDeviceId = DB::table('delivery_boy_new')
    		                       ->where('boy_phone', $phone)
    		                        ->update(['device_id'=>$device_id]);
